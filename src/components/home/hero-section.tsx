...
export function HeroSection() {
  return (
    <section className="relative min-h-screen flex flex-col justify-center overflow-hidden pt-20 pb-10">
      ...
      <div className="absolute top-1/4 left-1/4 w-96 h-96 bg-cyan-500/20 rounded-full blur-[120px] animate-pulse-glow pointer-events-none" />
      <div className="absolute bottom-1/4 right-1/4 w-96 h-96 bg-purple-500/20 rounded-full blur-[120px] animate-pulse-glow pointer-events-none" style={{ animationDelay: "1.5s" }} />

      {/* Abstract Orbs */}
      <div className="absolute top-1/3 right-1/4 w-32 h-32 rounded-full bg-gradient-to-tr from-cyan-500 to-blue-500 blur-xl opacity-50 animate-float pointer-events-none" />
      <div className="absolute bottom-1/3 left-1/4 w-24 h-24 rounded-full bg-gradient-to-bl from-purple-500 to-violet-500 blur-xl opacity-50 animate-float pointer-events-none" style={{ animationDelay: "2s", animationDuration: "8s" }} />
      ...
      <div className="container mx-auto px-4 relative z-10">
        ...
        >
          ...
          <motion.h1 variants={itemVariants} className="text-5xl md:text-7xl font-bold mb-4 tracking-tight">
            <span className="gradient-text neon-text-cyan block pb-2">XR Lab Aquarium</span>
          </motion.h1>

          <motion.h2 variants={itemVariants} className="text-2xl md:text-4xl font-medium text-gray-300 mb-6 font-sans" dir="rtl">
            فضای آموزشی آکواریوم
          </motion.h2>

          <motion.p variants={itemVariants} className="text-lg text-gray-400 mb-10 max-w-2xl mx-auto leading-relaxed">
            An immersive 2-month educational space where ~50 selected students dive deep into Virtual, Augmented, and Mixed Reality. Build real-world projects, get mentored by industry experts, and shape the future of spatial computing.
          </motion.p>
          ...
          >
            {stats.map((stat, i) => (
              <div key={i} className="glass rounded-xl p-6 flex flex-col items-center justify-center border-t-white/10 border-b-transparent border-l-transparent border-r-transparent shadow-lg transition-transform hover:-translate-y-1 hover:neon-purple">
